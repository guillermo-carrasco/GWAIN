...
//					std::cerr << "Error: OV_HOLE"     << std::endl;
//				else if(read_bytes == OV_EBADLINK)
//					std::cerr << "Error: OV_EBADLINK" << std::endl;
		}
		else {
			bytes  -= read_bytes;
			stream += read_bytes;
...
ALuint manager::source_reserve()
{
	...
	for(i = 0; i < NUM_SOURCES && m_vSourcesReserved[i]; i++);
...
void manager::source_unreserve(ALuint _srcID)
{
	...
	for(i = 0; i < NUM_SOURCES && m_vSources[i] != _srcID; i++);
...
//	if(processed) std::cerr << processed << std::endl;
...
bool stream::playing()
{
	ALint state;
	alGetSourcei(m_uiSource, AL_SOURCE_STATE, &state);
	return (state != AL_STOPPED) || (m_uiFlags & STREAM_OGG_PLAYING);
}
